...
| `gpt-5.6-terra` | **2 / 12** | 小模型池 2.5M tok/天 | 二者合计 ≈ **580**（充裕） |
| `gpt-5.6-luna` | **0.20 / 1.20** | 小模型池 2.5M tok/天 | 同上 |

> 价格为 **2026-07-30 降价后**（terra 原 2.50/15，-20%；luna 原 1/6，**-80%**）。
> 官方定价页对抓取器 403，价格取自三个独立二手源交叉验证 + OpenRouter 列表
> （其 `$1/$6` 带「50% off」促销标注，正好是新列表价 $2/$12 的五折，反向印证）。
> **降价对本 spec 的运营决策零影响**，逐条论证见 §2.2a。
...
### 2.2a luna/terra 降价复盘（2026-07-30）：为什么一行都不改

owner 提示 luna/terra 降价，逐条核过链上每个决策点，**结论是运营侧零改动**。
把论证记下来，是因为「降价了所以要优化」是个很自然但在这里不成立的直觉。

| 决策点 | 依据是什么 | 降价后 |
|---|---|---|
| terra/luna 排在 paid 前 | **免费池大小**（2.5M vs 250K），非单价 | 池没变 → **不变** |
| medical 段 terra → luna 的先后 | 二者**共用同一个 2.5M 池**，先用便宜的省不出任何免费额度 → 由**质量**定序 | **不变** |
| sol 只服务思考模式 | 稀缺（250K/58 次）+ 质量最高，配给最需要质量的场景 | sol 未降价 → **不变** |
| Google 免费打头 | ~9000 次/天是最大一份免费额度 | 与 OpenAI 定价无关 → **不变** |
| 付费层 = Google credits | **D002 立法意图「不为 OpenAI 付费」** | 见下 → **不变**（需 owner 裁决才动） |

**代码里没有任何逻辑读取价格** —— 三处价格全在注释。所以降价在实现层面是纯文档事件。

#### 降价确实带来的三个变化

1. **文档数字过期了。** 已修（本节表 + `api/ai-chat.ts` 注释块）。
   陈旧数字出现在**决策理由**里，会让后来者以为参数是按当前价算过的 ——
   与本项目反复出现的失效文档同形（见 `exportRedacted.ts` 坐标契约那次）。
2. **原有设计的张力消失了。** 原文要特意解释「主用 terra/luna 不是因为便宜，
   而是因为池大」—— 因为当时 luna($1/$6) 比 sol($5/$30) 便宜但不是最优模型，
   理由容易被误读成省钱。现在 luna 既在大池、单价又最低，**两个理由同向**。
3. **阶段 4 图片路径的 OpenAI 侧成本降 80%。** §4.4a 已确认 Gemini 图像 token
   固定（~1064，与像素无关）而 OpenAI 按 patch 线性计费 —— 后者现在便宜 5 倍。
   这不改变「Gemini 排第一」（它仍是零边际成本），但让 OpenAI 视觉降级层的
   成本顾虑基本消失。

#### 🔴 需要 owner 裁决的一条：D002 是否值得复议

D002 记的是「不为 OpenAI 付费」，付费层留在 Google credits。
**那个决定是在 luna = $1/$6 时做的。** 现在按 spec 基准（3.5K in + 0.8K out）：

| 付费层候选 | 单次成本 | $10 能撑 |
|---|---|---|
| `gemini-3.6-flash`（**当前**） | $0.0113 | ~890 次 |
| `gpt-5.6-luna`（新价） | **$0.0017** | **~6,000 次** |

**便宜 6.8 倍。** 付费层平时用不到（前面全是免费层），但它是**免费额度全部耗尽
时的最后一道非 backup 防线** —— 容量差 6.7 倍是实的。

⚠️ **我没有改，也不建议悄悄改**：
- D002 的立法意图不只是省钱，还有**不加深对单一供应商的依赖**。
  价格变了，但那层意图没变
- 需要 OpenAI 账号开通付费。当前只有一个 `OPENAI_API_KEY` 且是纯免费层，
  **池耗尽只会返回 429，没有付费可降** —— 换句话说这条改动需要账号侧动作，
  不是改代码就行
- 改付费层属治理决议变更，按 CONSTITUTION 应走 DECISIONS.md 修订

→ **要不要复议 D002，是你的决定。** 若决定复议，我可以出改动方案；
若维持现状，本节留作「已评估并主动否决」的记录，避免将来重复讨论。

### 2.2b 要不要做「按问题自动选模型」的智能路由（2026-07-30 评估：不做）

**先澄清：路由已经存在。** 现有三条链（think / medical / smalltalk）就是路由器，
分级逻辑在 `api/ai-chat.ts`：

```js
const GREETING_RE = /^(你好|您好|hi|hello|嗨|在吗|谢谢|…)[!！。.~？? ]*$/i;
const isSmallTalk = lastText.length <= 14 && GREETING_RE.test(lastText);
```

**锚定全串的白名单 + 14 字上限**，其余一律走医疗主力链。
注释里的设计原则是「保守窄召回：宁可把寒暄给主力模型，绝不把医疗问题给 lite」。

所以真问题是：**该不该做得更细。** 逐条查完，结论是**不做**。

#### 理由一：不是额度紧张（实测，非估算）

GA4 实测 30 天：**5,802 用户 / 7,948 会话 / 19,422 浏览** ≈ **193 用户/天**
（GA 在大陆被墙，真实更高，此为可测下界）。

| 场景 | 请求/天 | vs Google 免费 9000/天 |
|---|---|---|
| 激进假设：50% 日活用 chat × 每人 5 条 | ~483 | **余量 18.6×** |
| 要打满 Google 免费池 | 9000 | 需 **1800 chat 用户/天** = 现总流量的 **9.3 倍**且 100% 用 chat |

而这还只是 Google 免费层，后面还有 OpenAI 580/天 + 付费 + backup。
**「优化额度分配」是在解一个我们没有的问题。**

#### 理由二：最强的模型已经是默认

医疗链首位就是 `gemini-3.6-flash`（最好的免费 flash）。路由器**只可能把它降级**，
不可能升级 —— 除非升到 sol，而那撞理由三。

#### 理由三：自动升级到 sol 会毁掉它的保护

sol 池 250K tok/天 ≈ **58 次/天**。现有设计刻意让它**只服务用户主动开启的思考
模式**，理由写在 §2.2「不会被日常问答挤占」。自动升级会让日常问答几分钟内抽干
它 —— 恰好破坏那条保护，而且是无声的。

#### 理由四：LLM 路由撞本项目自己的原则

要「按语义自动选模型」，实做只有两条路：
- **扩正则/关键词** —— 便宜、确定、可审计，但解决的是理由一那个不存在的问题
- **加一次 LLM 分类调用** —— 这是通常说的「智能路由」，但在本架构里有四个硬伤：

  1. **花额度省额度**：分类调用本身消耗同一批池子
  2. **首字节前多一跳延迟** —— 而阶段 4 的 vision 已经在跟 25 秒 Edge 硬墙较劲
  3. **分类器自己也需要降级链**，否则它挂了整条路径就挂了
  4. 🔴 **在安全关键路径前面放一个不可验证的黑箱** ——
     `ai-chat-image-input.md` §7.1 已立过同一条原则：
     「判定同样用机器正则，**不用 LLM-as-judge**（医疗站不引入不可验证的评判黑箱）」。
     用 LLM 决定「这个可能是急症的问题交给哪个模型」，正是那条原则要挡的东西，
     且**误判是静默的**

#### 理由五：安全闸本来就不在路由上

`detectSomaticEmergency()` 在**客户端、模型调用之前、与模型无关**地跑
（见 `ai-chat-somatic-emergency.md`）。急症卡本地秒级渲染、不可关闭。
**换哪个模型都不影响急症识别** —— 路由是质量/成本问题，不是主安全闸。

#### 现有分级的安全性是构造保证的

`GREETING_RE` 锚定全串，所以「你好，我血钾 6.4」**不匹配**（有额外内容）→ 走医疗链。
白名单里全是纯问候语，不存在 ≤14 字且整串匹配的医疗内容。
另：think 模式 + 寒暄会走 smalltalk 链（`buildChain` 里 smalltalk 优先于 mode），
不会把 sol 浪费在「你好」上 —— 已处理。

#### 什么情况下这个结论会翻

不是永久否决，是**数据驱动的否决**。任一条成立就该重新评估：

| 触发条件 | 怎么测 |
|---|---|
| Google 免费层日常出现 `429` 降级 | `x-yk-route` 埋点里 free→free-oai 的降级率 > 5% |
| chat 请求 > 3000/天 | GA4 `ai_*` 事件（**目前为 0，因为分支未合并**） |
| sol 的 58 次/天被打满且用户抱怨思考模式不可用 | free-oai 层 `insufficient_quota` 频次 |
| 出现「同一问题不同模型答案质量差异显著」的实证 | 需要 A/B，目前无数据 |

⚠️ **前提是先把埋点合上线** —— 现在 GA4 里 `ai_*` 事件为 0，
上面这些指标一个都测不了。**在没有用量数据的情况下做路由优化，
是在没有仪表的情况下调发动机。**
