...
### 3.2 方案：降级为无单位相对示意（依 review #126 更新）

> **初版方案（谷值锚定）已被 review 否决，此处记录最终方案。**

初版曾用 `expectedE2Range` 谷值中点锚定纵轴、标成绝对 pg/mL。但 review #126 用硬数字
指出：只对齐一个谷值中点**救不了整条曲线** —— 本一室形状下 5 mg 峰值算出约 277 pg/mL，
而 Oriowo 原文 5 mg IM EV 平均峰值约 **667（95% CI 457–983）**，峰值被严重低估，
图上却还画着 100–200 目标带与 300 风险线，等于给用户一个可据以判断达标/超标的假刻度。

**最终方案**：把一室叠加形状**归一化到自身峰值**（0–1 相对），

```
peak = shape(2.4)            // 单针后峰约在 2–3 天（Oriowo）
rel(t) = shape(t) / peak     // 0–1 相对值
```

- y 轴**无数字刻度**，只标「峰 / 谷」方向；
- **移除** 100–200 目标带与 300 风险线；
- 说明明确写「纵轴是相对高度，**不是 pg/mL**。峰值绝对高度取决于剂量与个体，本图不作声称」。

曲线只教一件有据且不涉及绝对值的事：血药浓度**何时高、何时低**（注射后 2–3 天达峰，
下次注射前为谷）→ 所以**血检要采谷值**。形状与剂量无关（同一条），对所有剂量一致展示。

绝对浓度信息仍在左卡以数据形式呈现（`expectedE2Range`，来自 injection-doses.json，
是有来源的真值），但**不再画进带假刻度的曲线**。
...
2. **§3.2** —— 曲线已按 review #126 降级为无单位相对示意（去掉目标带/风险线/绝对纵轴）。
   请确认这个降级是否达到「不再声称任何未经验证的绝对浓度」的要求。
3. **§2.4a** —— 空初始值 vs 原型的演示默认值。
4. **review #2** —— 血检已补「假设正在接受 HRT + 按化验单 ULN」的上下文前提横幅，
   并把裁定口吻从「危险/停药」改为「对照目标区间/就医评估」。是否足够？
   还是要进一步加「治疗阶段」输入才允许出红区判读？
