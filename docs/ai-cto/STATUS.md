...
- **v2.2 视觉重做（「新版」绯英典籍手账）**：owner 再次否决 v2.1 白底玫红（「按照新版的设计，不是这个老版本」）→ 按 `docs/specs/brand-library-v2.2-sakura.md` 用交接包 `Primitives.jsx` / `layout.css` 配方重写展示层：线装卷页头（竖排「卷二・图鉴」+ washi kicker + display 字体 h1 + 红结墨线）、cream 搜索条 + pink-aa 按钮 + 手写提示、ivory InkCard 筛选栏（自绘复选框 + 狐狸老师气泡）、墨描边硬投影品牌族卡片（网点纸图片区 + 旋转贴纸角标 + 类别 Chip + 点线页脚）、禁用卡红章 + 墨底 DangerBox、gold WarningBox 核对条、MangaPanel 对话框。`.bl-root` 自带 `--yk-*` token（站点变量→交接包字面值回退），**不依赖 🌸 开关**；暗色按幻月夜模型（桌面紫、纸面亮）。36/36 测试绿。
- **v2.1 视觉重构（owner 参考稿，已被 v2.2 覆盖）**：owner 否决「暗色药典图版」方向并给出浅色目录式参考稿 → 按 `docs/specs/brand-library-v2.1-visual.md` 逐区重构展示层：浅色白底 + 左侧复选框多选筛选栏（地区/类别/剂型/参考资料 + 全部地区/重置）+ 大搜索条与玫红按钮 + 右上核对提示卡 + 信息条 + 「品牌图鉴」排序/视图切换 + 3 列白底圆角卡片（**品牌族**：141 版本归并为 111 张卡，详情内切换版本）+ 每卡「包装示意」（新 `PackArt.tsx`：药盒 + 泡罩/剂型件，无厂商 logo）或实拍 + 底部核对提示条；外观反查折叠为「按外观查找」；dark/sakura 同版式换面。mdx 页顶去掉重复导语与返回链接，ToolDisclaimer 移到岛屿下方。默认排序按成分（同成分内多版本/有实拍优先）。36/36 测试绿。
