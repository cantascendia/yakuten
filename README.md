...
[![Vercel](https://img.shields.io/badge/Deployed%20on-Vercel-black?style=flat-square&logo=vercel)](https://hrtyaku.com)
...
[![i18n](https://img.shields.io/badge/Languages-中文%20%7C%20English%20%7C%20日本語%20%7C%20한국어-blue?style=flat-square)](#languages)
...
- AI 生成的医学示意图（14 张 × 4 语言）和药物产品照（7 张）
...
| **Drug Comparator** | 药物对比 — 20 种药物多维度横向对比 |
| **Risk Screener** | 风险自评 — 7 问卷 4 维度风险评分 (VTE/肝脏/脑膜瘤/心血管) |
| **AI Assistant** | AI 问答 — 基于站内文献库的智能助手 |
| **Reference Library** | 参考文献库 — 27 条核心文献检索 |
| **Hospital Finder** | 友好医疗资源 — 全国 25 家跨性别友好医院 |

### SEO 博客

11 篇长尾关键词文章，覆盖「补佳乐怎么吃」「色普龙副作用」等高搜索量话题，引导用户进入循证内容体系。
...
- AI 对话不存储任何记录
- 第三方分析仅采集聚合 PV/事件（Vercel Analytics + Google Analytics 4），绝不上报健康数据、用户输入或血检记录，无广告/社媒像素
- GA 的 gtag 在中国大陆被墙，大陆流量以 Bing 站长 + Vercel Analytics（均可在墙内访问）为准
...
Pagefind                 Offline-capable quadrilingual search
CSS Variables            Zero Tailwind, miHoYo-inspired design system
Gemini Flash Image       AI-generated medical diagrams (Nano Banana 2)
```
...
│   │   ├── ja/          # 日本語 (44 pages)
│   │   └── ko/          # 한국어 (44 pages)
│   └── blog/zh/         # SEO blog articles (11 posts)
├── data/
│   ├── drugs.json       # Structured drug database (20 drugs)
│   ├── blood-ranges.json# Blood test reference ranges
│   ├── references.json  # Citation database (27 refs)
│   └── hospitals.json   # Trans-friendly hospital database (25 hospitals)
├── i18n/
│   └── ui.ts            # Quadrilingual UI strings
public/
└── images/
    ├── drugs/           # AI-generated product photos (7 images)
    ├── diagrams/        # Medical diagrams, English base
    └── diagrams/{zh,en,ja,ko}/  # Localized medical diagrams
scripts/
├── generate-images.mjs  # Nano Banana 2 batch image generator
