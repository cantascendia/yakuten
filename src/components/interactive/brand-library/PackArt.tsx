/**
 * 药物图鉴 v2.2 — 包装示意图（PackArt）
 * spec: docs/specs/brand-library-v2.2-sakura.md §2.6（描边改梅子墨 2px、色带用类别色、铝箔用 ivory）
 *
...
function pocketShape(
  ...
  if (shape === 'oval' || shape === 'oblong' || shape === 'capsule') {
    return <ellipse cx={cx} cy={cy} rx={15} ry={9} fill={fill} stroke={stroke} strokeWidth={1.5} />;
  }
  return <circle cx={cx} cy={cy} r={11.5} fill={fill} stroke={stroke} strokeWidth={1.5} />;
}
...
export default function PackArt({
  ...
  const textStyle: CSSProperties = { fontFamily: 'var(--yk-font-body)' };
  ...
  return (
    ...
    >
      ...
      {/* 顶面 / 右侧面：微透视 */}
      <path d="M34 98 L58 80 L278 80 L254 98 Z" fill={faceTop} stroke={line} strokeWidth="2" strokeLinejoin="round" />
      <path
      ...
      {/* 正面 */}
      <rect x="34" y="98" width="220" height="150" rx="6" fill={face} stroke={line} strokeWidth="2" />
      <g clipPath={`url(#${clipId})`}>
        <rect x="34" y="98" width="220" height="22" fill={band} />
      </g>
      <line x1="34" y1="120" x2="254" y2="120" stroke={line} strokeWidth="2" />
      ...
      <line x1="50" y1="212" x2="176" y2="212" stroke={line} strokeWidth="4" strokeLinecap="round" opacity="0.3" />
      <line x1="50" y1="226" x2="140" y2="226" stroke={line} strokeWidth="4" strokeLinecap="round" opacity="0.18" />

      {blister ? (
        <g>
          {/* 泡罩板：铝箔底 + 2×5 pocket */}
          <rect x="150" y="168" width="214" height="114" rx="10" fill={foil} stroke={line} strokeWidth="2" />
          {[0, 1].map((row) =>
            [0, 1, 2, 3, 4].map((col) => {
